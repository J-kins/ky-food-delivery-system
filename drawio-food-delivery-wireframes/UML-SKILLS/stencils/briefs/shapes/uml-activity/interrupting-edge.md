...
|----------|---------|---------|
| Label | **InputText** | `Interrupting Edge` |
...
- Tile `aria-label="Interrupting Edge"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-activity/interrupting-edge.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#interrupting-edge`
