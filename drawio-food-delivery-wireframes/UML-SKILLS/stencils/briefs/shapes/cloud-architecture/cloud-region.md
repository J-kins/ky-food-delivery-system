...
- **Goal:** Cloud region
- **Description:** Box with region name
- **Visual notation:** `us-east-1`
...
- Tile `aria-label="Cloud Region"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/cloud-architecture/cloud-region.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#cloud-region`
