...
- **Description:** Dashed arrow <<merge>>
- **Visual notation:** `─ ─ ▶ <<merge>>`
...
- Tile `aria-label="Package Merge"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-package/package-merge.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#package-merge`
