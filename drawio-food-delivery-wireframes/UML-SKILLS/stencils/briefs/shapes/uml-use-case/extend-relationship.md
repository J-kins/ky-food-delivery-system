...
- **Goal:** Optional functionality
- **Description:** Dashed arrow <<extend>>
- **Visual notation:** `─ ─ ▶ <<extend>>`
...
- Tile `aria-label="Extend"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-use-case/extend-relationship.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#extend-relationship`
