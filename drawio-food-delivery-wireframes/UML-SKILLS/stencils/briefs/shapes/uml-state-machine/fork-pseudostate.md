...
|----------|---------|---------|
| Label | **InputText** | `Fork Pseudo-state` |
...
- Tile `aria-label="Fork Pseudo-state"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-state-machine/fork-pseudostate.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#fork-pseudostate`
