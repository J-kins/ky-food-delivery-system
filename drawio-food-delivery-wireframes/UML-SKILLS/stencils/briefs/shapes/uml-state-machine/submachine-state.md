...
- **Goal:** Reusable state
- **Description:** State with ref
- **Visual notation:** `┌ State ref ┘`
...
|----------|---------|---------|
| Label | **InputText** | `Submachine State` |
...
- Tile `aria-label="Submachine State"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-state-machine/submachine-state.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#submachine-state`
