...
- **Goal:** Starting point
- **Description:** Solid black circle
- **Visual notation:** `●`
...
- Tile `aria-label="Initial State"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-state-machine/sm-initial-state.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#sm-initial-state`
