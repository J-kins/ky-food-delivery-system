...
- **Goal:** Nested states
- **Description:** State with internal border
- **Visual notation:** `┌ State ┌─┐ ┘`
...
|----------|---------|---------|
| Label | **InputText** | `Composite State` |
...
- Tile `aria-label="Composite State"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-state-machine/composite-state.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#composite-state`
