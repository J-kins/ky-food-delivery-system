# Entry Point — Shape Design Brief

**Project:** UML-SKILLS Stencil Toolkit  
**Category:** UML State Machine Diagram (`uml-state-machine`)  
**Shape ID:** `entry-point`  
**Asset type:** `marker` | **Delivery:** `manual`  
**SVG:** `../svg/shapes/uml-state-machine/entry-point.svg`  
**Version:** 1.0  
...
- **Goal:** Entry into composite
- **Description:** Circle with arrow
- **Visual notation:** `●→`
...
- **Diagram types:** UML State Machine Diagram diagrams.
- **Palette tile:** 48×48 **card** tile in `uml-state-machine` **accordion** section.
- **Glossary terms:** **State**, **Transition**, **Visual Hierarchy**
- **Notes:** Hand-crafted SVG required — follow [STYLE_GUIDE.md](../svg/STYLE_GUIDE.md). 
...
- Tile `aria-label="Entry Point"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-state-machine/entry-point.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#entry-point`
