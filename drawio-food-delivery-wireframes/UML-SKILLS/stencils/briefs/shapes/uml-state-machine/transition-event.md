...
- **Goal:** Triggering event
- **Description:** Text on transition
- **Visual notation:** `event()`
...
- Tile `aria-label="Event"`; canvas shape `role="img"` with descriptive label.
...
- [ ] Listed in `sprite.svg` symbol `#transition-event`
