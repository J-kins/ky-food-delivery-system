...
- **Goal:** User timeline
- **Description:** Actor with dashed line
- **Visual notation:** `👤 │ │ │`
...
|----------|---------|---------|
| Label | **InputText** | `Actor Lifeline` |
...
- Tile `aria-label="Actor Lifeline"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-sequence/actor-lifeline.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#actor-lifeline`
