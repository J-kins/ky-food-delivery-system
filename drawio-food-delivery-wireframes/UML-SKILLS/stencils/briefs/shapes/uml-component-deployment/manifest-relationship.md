...
- **Goal:** Artifact manifestation
- **Description:** Dashed arrow <<manifest>>
- **Visual notation:** `─ ─ ▶ <<manifest>>`
...
- Tile `aria-label="Manifest"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-component-deployment/manifest-relationship.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#manifest-relationship`
