...
- **Goal:** Deployment relationship
- **Description:** Dashed arrow <<deploy>>
- **Visual notation:** `─ ─ ▶ <<deploy>>`
...
- Tile `aria-label="Deployment"`; canvas shape `role="img"` with descriptive label.
- **Keyboard accessible** move/resize when selected.
- **Contrast ratio** check for text inside shape bounds.

## 8. Deliverables Checklist

- [ ] SVG asset at `../svg/shapes/uml-component-deployment/deployment-relationship.svg`
- [ ] Palette thumbnail (default + **hover** + **focus**)
- [ ] Canvas instance (default + selected)
- [ ] Listed in `sprite.svg` symbol `#deployment-relationship`
