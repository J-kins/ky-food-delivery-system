"""Aspose.Diagram helpers for CPM network rendering."""
...
def add_cpm_node(
    ...
    try:
        ...
        log.debug("  [node %03d] addText failed: %s", sid, exc)
        return sid
...
def add_text_box(page, x: float, y: float, w: float, h: float, text: str, font_size: float = 11.0) -> None:
    add_cpm_node(
        page,
        x,
        y,
...
def save_diagram(diagram, output_path: str) -> None:
    ...
    log.info("CPM diagram saved to %s (%d bytes)", output_path, os.path.getsize(output_path))
