# Business Process Model — Template Design Brief

**Project:** UML-SKILLS Diagram Template Library  
**Category:** Process & Flow Diagrams (`process-flow`)  
**Template ID:** `business-process-model`  
**Layout:** `bpmn_swimlanes`  
**SVG:** `../svg/process-flow/57-business-process-model-template.svg`  
**Version:** 1.0  
...
- **Goal:** Provide a ready-made blank canvas for **Business Process Model** so users begin with correct structure, guides, and placeholder regions.
- **Description:** BPMN swimlanes, tasks, gateways
...
- **When to use:** Starting a new **Process & Flow Diagrams** diagram without building layout from scratch.
- **User flow:** Gallery card → **wizard** confirms name → canvas opens → user replaces placeholders with stencils from the shape library.
- **Glossary terms:** **Swimlane**, **Flow**, **Decision**
...
| Template title | read-only **InputText** | `Business Process Model` |
...
- Placeholders map to stencil categories from `process-flow` and related shape packs.
...
- Canvas `aria-label="Business Process Model template for project diagram"`.
- Each placeholder region named (e.g. "Class A placeholder").
- **Keyboard accessible** navigation between placeholders (Tab cycle).

## 10. Deliverables Checklist

- [ ] SVG at `../svg/process-flow/57-business-process-model-template.svg`
