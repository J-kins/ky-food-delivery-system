# Service Mesh Architecture — Template Design Brief

**Project:** UML-SKILLS Diagram Template Library  
**Category:** DevOps & CI/CD Diagrams (`devops`)  
**Template ID:** `service-mesh-architecture`  
**Layout:** `service_mesh`  
**SVG:** `../svg/devops/86-service-mesh-architecture-template.svg`  
**Version:** 1.0  
...
- **Goal:** Provide a ready-made blank canvas for **Service Mesh Architecture** so users begin with correct structure, guides, and placeholder regions.
- **Description:** Control plane, data plane, sidecars
...
- **When to use:** Starting a new **DevOps & CI/CD Diagrams** diagram without building layout from scratch.
- **User flow:** Gallery card → **wizard** confirms name → canvas opens → user replaces placeholders with stencils from the shape library.
- **Glossary terms:** **Pipeline**, **Stage**, **Loop**
...
| Template title | read-only **InputText** | `Service Mesh Architecture` |
...
- Placeholders map to stencil categories from `devops` and related shape packs.
...
- Canvas `aria-label="Service Mesh Architecture template for project diagram"`.
- Each placeholder region named (e.g. "Class A placeholder").
- **Keyboard accessible** navigation between placeholders (Tab cycle).

## 10. Deliverables Checklist

- [ ] SVG at `../svg/devops/86-service-mesh-architecture-template.svg`
