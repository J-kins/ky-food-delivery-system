# Cloud Migration Architecture — Template Design Brief

**Project:** UML-SKILLS Diagram Template Library  
**Category:** Cloud Architecture Diagrams (`cloud`)  
**Template ID:** `cloud-migration-architecture`  
**Layout:** `migration`  
**SVG:** `../svg/cloud/79-cloud-migration-architecture-template.svg`  
**Version:** 1.0  
...
- **Goal:** Provide a ready-made blank canvas for **Cloud Migration Architecture** so users begin with correct structure, guides, and placeholder regions.
- **Description:** On-premises to cloud transition
...
- **When to use:** Starting a new **Cloud Architecture Diagrams** diagram without building layout from scratch.
- **User flow:** Gallery card → **wizard** confirms name → canvas opens → user replaces placeholders with stencils from the shape library.
- **Glossary terms:** **Region**, **Service**, **Grid**
...
| Template title | read-only **InputText** | `Cloud Migration Architecture` |
...
- Placeholders map to stencil categories from `cloud` and related shape packs.
...
- Canvas `aria-label="Cloud Migration Architecture template for project diagram"`.
- Each placeholder region named (e.g. "Class A placeholder").
- **Keyboard accessible** navigation between placeholders (Tab cycle).

## 10. Deliverables Checklist

- [ ] SVG at `../svg/cloud/79-cloud-migration-architecture-template.svg`
