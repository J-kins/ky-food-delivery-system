# High Availability Architecture — Template Design Brief

**Project:** UML-SKILLS Diagram Template Library  
**Category:** Infrastructure Diagrams (`infrastructure`)  
**Template ID:** `high-availability-architecture`  
**Layout:** `ha_active_standby`  
**SVG:** `../svg/infrastructure/37-high-availability-architecture-template.svg`  
**Version:** 1.0  
...
- **Goal:** Provide a ready-made blank canvas for **High Availability Architecture** so users begin with correct structure, guides, and placeholder regions.
- **Description:** Active, standby, and failover paths
...
- **When to use:** Starting a new **Infrastructure Diagrams** diagram without building layout from scratch.
- **User flow:** Gallery card → **wizard** confirms name → canvas opens → user replaces placeholders with stencils from the shape library.
- **Glossary terms:** **Network**, **Topology**, **Container**
...
| Template title | read-only **InputText** | `High Availability Architecture` |
...
- Placeholders map to stencil categories from `infrastructure` and related shape packs.
...
- Canvas `aria-label="High Availability Architecture template for project diagram"`.
- Each placeholder region named (e.g. "Class A placeholder").
- **Keyboard accessible** navigation between placeholders (Tab cycle).

## 10. Deliverables Checklist

- [ ] SVG at `../svg/infrastructure/37-high-availability-architecture-template.svg`
