# Data Lakehouse Architecture — Template Design Brief

**Project:** UML-SKILLS Diagram Template Library  
**Category:** Data & Database Diagrams (`data`)  
**Template ID:** `data-lakehouse-architecture`  
**Layout:** `lakehouse_layers`  
**SVG:** `../svg/data/68-data-lakehouse-architecture-template.svg`  
**Version:** 1.0  
...
- **Goal:** Provide a ready-made blank canvas for **Data Lakehouse Architecture** so users begin with correct structure, guides, and placeholder regions.
- **Description:** Bronze, Silver, Gold medallion layers
...
- **When to use:** Starting a new **Data & Database Diagrams** diagram without building layout from scratch.
- **User flow:** Gallery card → **wizard** confirms name → canvas opens → user replaces placeholders with stencils from the shape library.
- **Glossary terms:** **Data Grid**, **Entity**, **Relationship**
...
| Template title | read-only **InputText** | `Data Lakehouse Architecture` |
...
- Placeholders map to stencil categories from `data` and related shape packs.
...
- Canvas `aria-label="Data Lakehouse Architecture template for project diagram"`.
- Each placeholder region named (e.g. "Class A placeholder").
- **Keyboard accessible** navigation between placeholders (Tab cycle).

## 10. Deliverables Checklist

- [ ] SVG at `../svg/data/68-data-lakehouse-architecture-template.svg`
