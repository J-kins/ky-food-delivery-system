# UML Interaction Overview Diagram — Template Design Brief

**Project:** UML-SKILLS Diagram Template Library  
**Category:** UML Diagrams (`uml`)  
**Template ID:** `uml-interaction-overview`  
**Layout:** `interaction_overview`  
**SVG:** `../svg/uml/12-uml-interaction-overview-template.svg`  
**Version:** 1.0  
...
- **Goal:** Provide a ready-made blank canvas for **UML Interaction Overview Diagram** so users begin with correct structure, guides, and placeholder regions.
- **Description:** Activity nodes referencing interaction fragments
...
- **When to use:** Starting a new **UML Diagrams** diagram without building layout from scratch.
- **User flow:** Gallery card → **wizard** confirms name → canvas opens → user replaces placeholders with stencils from the shape library.
- **Glossary terms:** **Card**, **Container**, **Visual Hierarchy**
...
| Template title | read-only **InputText** | `UML Interaction Overview Diagram` |
...
- Placeholders map to stencil categories from `uml` and related shape packs.
...
- Canvas `aria-label="UML Interaction Overview Diagram template for project diagram"`.
- Each placeholder region named (e.g. "Class A placeholder").
- **Keyboard accessible** navigation between placeholders (Tab cycle).

## 10. Deliverables Checklist

- [ ] SVG at `../svg/uml/12-uml-interaction-overview-template.svg`
