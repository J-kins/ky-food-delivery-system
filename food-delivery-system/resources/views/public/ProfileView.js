...
export function ProfileView() {
  ...
  const nameInput = Input({ type: 'text', label: 'Full Name', value: 'John Doe' });
  const emailInput = Input({ type: 'email', label: 'Email', value: 'john@example.com' });
  const phoneInput = Input({ type: 'tel', label: 'Phone', value: '+256 700 123456' });

  const saveBtn = Button({ label: 'Save Changes', variant: 'primary', size: 'lg' });
