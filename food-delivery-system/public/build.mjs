...
const buildConfig = {
  ...
  define: {
    ...
    '__SHOW_FOOTER__': JSON.stringify(process.env.TARGET !== 'tauri'),
    '__SHOW_NAVBAR__': 'true',
    '__SHOW_SIDEBAR__': JSON.stringify(process.env.TARGET === 'tauri' || process.env.TARGET === 'web'),
...
function injectBundleIntoHtml() {
  ...
  html = html.replace(/<script\s+src="\/dist\/public\/bundle\.js"><\/script>/g, '');
